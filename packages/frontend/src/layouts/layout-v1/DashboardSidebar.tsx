...
type activeMenu = { active: boolean };
// --------------------------------------------------------------

// custom styled components
const SidebarWrapper = styled(Box)<{ show: boolean }>(({ theme, show }) => ({
  left: show ? 0 : -260,
  width: 260,
  height: "100%",
  position: "fixed",
...
const StyledListItemButton = styled(ListItemButton)<{ active: string }>(
  ({ active, theme }) => ({
    padding: 12,
    marginBottom: 8,
...
const DashboardSidebar: FC<SideNavBarProps> = (props) => {
  ...
  const handleAccordionChange =
    (panel: string) => (_: SyntheticEvent, expand: boolean) => {
  ...
  const activeRoute = (path: string) => (path === pathname ? true : false);
