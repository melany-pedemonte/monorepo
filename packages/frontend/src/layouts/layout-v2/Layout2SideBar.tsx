...
import { Box, List, ListItem, styled, Theme, Tooltip } from "@mui/material";
...
const SubMenuItem = styled(FlexBox)<{ active?: boolean }>(
  ({ theme, active }) => ({
    ...
    "&:hover": {
      ...
      "& #name": { color: theme.palette.primary.main },
    },
  })
);

// root component
const Layout2SideBar: FC<SideNavBarProps> = (props) => {
  const {
    sideBarLocked,
    showMobileSideBar,
    closeMobileSideBar,
  ...
  const activeRoute = () => {
    sideMenuList.forEach((menu) => {
      if (menu.title === "Dashboards") {
        // @ts-ignore
        const match = menu.children.find((item: any) => item.path === pathname);
        if (match) {
